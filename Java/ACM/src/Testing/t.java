...
public class t
{
    ...
    static int second(int y, int m, int d) {
		...
		for (int i = 1582; i <= y - 1; i++)
			if (gregorianLeapYear(i))
				sum += 366;
			else
				sum += 365;
		int a[] = { 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 };
		if (gregorianLeapYear(y))
			a[1]++;
		for (int i = 1; i <= m - 1; i++) {
			sum += a[i-1];
		}
    ...
    public static void main(String[] args)
    {
        try {
            ...
            while (s != null) {
                ...
                System.out.println(first(jy, jm, jd));
                System.out.println(second(jy, jm, jd));
                System.out.println(first(jy, jm, jd) - second(jy, jm, jd));
